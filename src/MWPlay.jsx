...
export default function MWPlay(){
  ...
  const handleVerifySubmit=async request=>{
    ...
    // Persist to Supabase — store reason + status in profile
    if(isSupabaseReady){
      db.updateOwnProfile(currentUser.id,{
        verification_status:"Pending",
        bio:currentUser.bio||(request.reason?`Verification request: ${request.reason}`:undefined),
      }).catch(()=>{});
    }
  };
