...
const UPLOAD_FEE = 2500;
const UPLOAD_FEE_LABEL = "MWK 2,500";
...
// ─── PAYMENT MODAL (TNM + AIRTEL) ─────────────────────────────────────────────
async function initiateMobilePayment(provider, phone, amount, ref) {
  // In production: call your backend which calls the real API
  // TNM Mpamba API: https://mpamba.tnm.mw/api
  // Airtel Money API: https://developer.airtel.africa/
  await new Promise(r => setTimeout(r, 1800));
  return { transaction_id: "TXN-" + Date.now(), status: "PENDING" };
}
async function pollPaymentStatus(txnId, attempt) {
  await new Promise(r => setTimeout(r, 4500));
  return { status: attempt >= 2 ? "COMPLETED" : "PENDING" };
}
...
function PaymentModal({ amount, amountLabel, purpose, onSuccess, onClose }) {
  ...
  const [state, setState] = useState("idle"); // idle|initiating|pending|success|failed
  const [txnId, setTxnId] = useState(null);
  const [attempt, setAttempt] = useState(0);
  const pollRef = useRef(null);

  const PROVIDERS = {
    tnm:   { name:"TNM Mpamba",   color:"#E63946", prefix:"088", logo:"📱" },
    airtel:{ name:"Airtel Money", color:"#E63946", prefix:"099", logo:"📲" },
  };

  useEffect(() => () => clearTimeout(pollRef.current), []);

  useEffect(() => {
    if (state !== "pending" || !txnId) return;
    const poll = async (att) => {
      const res = await pollPaymentStatus(txnId, att);
      if (res.status === "COMPLETED") { setState("success"); onSuccess(txnId); }
      else if (res.status === "FAILED") { setState("failed"); }
      else {
        const next = att + 1;
        if (next > 12) { setState("failed"); return; }
        setAttempt(next);
        pollRef.current = setTimeout(() => poll(next), 100);
      }
    };
    pollRef.current = setTimeout(() => poll(attempt), 100);
  }, [state, txnId]);

  const validate = () => {
    const c = phone.replace(/[\s-]/g, "");
    const p = PROVIDERS[provider];
    if (!/^(0|265)[0-9]{8,9}$/.test(c)) { setPhoneErr(`Enter a valid ${p.name} number`); return false; }
    return true;
  };
  ...
  const initPay = async () => {
    ...
    setState("initiating");
    try {
      const res = await initiateMobilePayment(provider, phone, amount, "MWPLAY-"+genId());
      setTxnId(res.transaction_id); setAttempt(0); setState("pending");
  ...
  if (state === "success") return (
    <Modal onClose={onClose} maxWidth={360}>
      <div style={{textAlign:"center",padding:"16px 0"}}>
        ...
        <h3 style={{color:T.text,fontFamily:"'Syne',sans-serif",fontSize:20,marginBottom:8}}>Payment confirmed!</h3>
        <p style={{color:T.sub,fontSize:13,lineHeight:1.7,marginBottom:6}}><span style={{color:T.green,fontWeight:700}}>{amountLabel}</span> received via {PROVIDERS[provider].name}.</p>
        <div style={{background:T.bg2,border:`1px solid ${T.border}`,borderRadius:8,padding:"10px 14px",margin:"16px 0 20px",textAlign:"left"}}>
          <div style={{color:T.mute,fontSize:10,letterSpacing:.5,marginBottom:3}}>TRANSACTION ID</div>
          <div style={{color:T.sub,fontSize:11,fontFamily:"'DM Mono',monospace"}}>{txnId}</div>
        </div>
  ...
  return (
    <Modal onClose={onClose} maxWidth={400}>
      <ModalHeader title="Mobile Payment" sub={purpose} onClose={onClose}/>
      <div style={{display:"flex",gap:6,marginBottom:20}}>
        {Object.entries(PROVIDERS).map(([k,v]) => (
          <button key={k} style={{flex:1,padding:"12px 8px",borderRadius:10,border:`1px solid ${provider===k?v.color+"66":T.border}`,background:provider===k?v.color+"18":"transparent",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",transition:"all .15s"}} onClick={()=>{setProvider(k);setPhoneErr("");}}>
            <div style={{fontSize:22,marginBottom:4}}>{v.logo}</div>
            <div style={{color:provider===k?T.text:T.sub,fontSize:11,fontWeight:700}}>{v.name}</div>
          </button>
        ))}
      </div>
      <div style={{textAlign:"center",padding:"14px 0",borderTop:`1px solid ${T.border}`,borderBottom:`1px solid ${T.border}`,marginBottom:18}}>
        <div style={{color:T.mute,fontSize:10,letterSpacing:1,marginBottom:4}}>AMOUNT</div>
        <div style={{color:T.text,fontFamily:"'Syne',sans-serif",fontSize:32,fontWeight:700,letterSpacing:-1}}>{amountLabel}</div>
      </div>

      {state === "idle" && <>
        <Field label={`${PROVIDERS[provider].name} number`}>
          <div style={{position:"relative"}}>
            <span style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",color:T.sub,fontSize:12,fontFamily:"'DM Mono',monospace",userSelect:"none"}}>+265</span>
            <input style={{...inp(),paddingLeft:52,fontFamily:"'DM Mono',monospace",letterSpacing:1}} value={phone} onChange={e=>{setPhone(e.target.value);setPhoneErr("");}} placeholder={PROVIDERS[provider].prefix+" XXX XXX"} type="tel" inputMode="numeric"/>
          </div>
          {phoneErr && <p style={{color:T.orange,fontSize:11,marginTop:4}}>{phoneErr}</p>}
        </Field>
        <p style={{color:T.mute,fontSize:11,lineHeight:1.6,marginBottom:16}}>You'll receive a USSD prompt on your phone. Enter your PIN to confirm.</p>
        <Btn full color={T.green} onClick={initPay}>Pay {amountLabel} →</Btn>
      </>}

      {state === "initiating" && <div style={{textAlign:"center",padding:"32px 0"}}><div style={{width:40,height:40,border:`2px solid ${T.border2}`,borderTopColor:T.blue,borderRadius:"50%",margin:"0 auto 14px",animation:"mwspin .7s linear infinite"}}/><p style={{color:T.sub,fontSize:13}}>Sending USSD push...</p></div>}

      {state === "pending" && <div style={{textAlign:"center",padding:"16px 0"}}>
        <div style={{fontSize:36,marginBottom:12}}>📲</div>
        <p style={{color:T.text,fontSize:14,fontWeight:600,marginBottom:6}}>Check your phone</p>
        <p style={{color:T.sub,fontSize:12,lineHeight:1.6,marginBottom:18}}>Enter your {PROVIDERS[provider].name} PIN to confirm <strong style={{color:T.text}}>{amountLabel}</strong>.</p>
        <div style={{display:"flex",alignItems:"center",gap:8,justifyContent:"center",color:T.mute,fontSize:11,marginBottom:20}}>
          <div style={{width:6,height:6,borderRadius:"50%",background:T.orange,animation:"mwpulse 1.2s ease-in-out infinite"}}/>Waiting for PIN...
        </div>
        <BtnGhost onClick={()=>setState("idle")}>Cancel</BtnGhost>
      </div>}

      {state === "failed" && <div style={{textAlign:"center",padding:"16px 0"}}>
        <div style={{fontSize:36,marginBottom:12}}>❌</div>
        <p style={{color:T.text,fontSize:14,fontWeight:600,marginBottom:6}}>Payment failed</p>
        <p style={{color:T.sub,fontSize:12,marginBottom:20}}>Insufficient funds, wrong PIN, or timeout. Please try again.</p>
        <Btn color={T.orange} onClick={()=>{setState("idle");setAttempt(0);}}>Try again</Btn>
      </div>}
    </Modal>
  );
}
...
function AlbumUploadModal({ onClose, currentUser, onAlbumUpload, toast }) {
  ...
  const submit = txnId => {
    const albumId = genId();
    const newSongs = tracks.map((t,i) => ({
      id: genId(), albumId, albumName: albumInfo.title,
      title: t.title||`Track ${i+1}`, artist: currentUser.name, artistId: currentUser.id,
      genre: albumInfo.genre, cover: cover||`https://picsum.photos/seed/${albumId}/300/300`,
      audioUrl: t.file?URL.createObjectURL(t.file):null,
      duration: t.duration?fmtTime(t.duration):"—", durationSecs:t.duration||0,
...
function VerifyEmailScreen({email,onVerified,onResend,onBack}){
  ...
  const handleChange=(val,idx)=>{
    ...
    if(next.every(c=>c)){if(next.join("")==="947182")setTimeout(()=>onVerified(),300);else setError("Wrong code. Demo: 947182");}
  };
  const handleKey=(e,idx)=>{if(e.key==="Backspace"&&!code[idx]&&idx>0)inputs.current[idx-1]?.focus();};
  return(
    <div style={{minHeight:"100vh",background:T.bg,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{width:"100%",maxWidth:380,background:T.bg1,border:`1px solid ${T.border}`,borderRadius:20,padding:"40px 28px"}}>
        ...
        <div style={{display:"flex",gap:8,justifyContent:"center",marginBottom:20}}>
          {code.map((c,i)=>(
            <input key={i} ref={el=>inputs.current[i]=el} maxLength={1} value={c} onChange={e=>handleChange(e.target.value,i)} onKeyDown={e=>handleKey(e,i)}
              style={{width:44,height:52,textAlign:"center",background:T.bg2,border:`1px solid ${c?T.blue:T.border}`,borderRadius:10,color:T.text,fontSize:20,fontWeight:700,outline:"none",transition:"border-color .15s"}}/>
          ))}
        </div>
        ...
        <p style={{color:T.mute,fontSize:11,textAlign:"center",marginTop:10}}>Demo code: <strong style={{color:T.sub}}>947182</strong></p>
      </div>
    </div>
  );
}
...
function AuthModal({mode:initMode,onClose,onAuth,registeredUsers}){
  ...
  const handleSubmit=()=>{
    ...
    if(mode==="signup"){
      ...
      if(registeredUsers.find(u=>u.email===form.email))return setError("Account with this email already exists.");
      setLoading(true);setTimeout(()=>{setLoading(false);setAwaitVerify(true);},700);
    }else{
      ...
      setLoading(true);setTimeout(()=>{setLoading(false);onAuth(user);onClose();},500);
    }
  };
  ...
  if(awaitVerify)return(
    <div style={{position:"fixed",inset:0,zIndex:600}}>
      <VerifyEmailScreen email={form.email}
        onVerified={()=>{
          const newUser={id:genId(),name:form.name,email:form.email,password:form.password,
        ...
        onResend={()=>{}} onBack={()=>setAwaitVerify(false)}/>
    </div>
  );
...
function UploadModal({onClose,currentUser,onUpload,toast}){
  ...
  if(showPay)return(
    <PaymentModal amount={UPLOAD_FEE} amountLabel={UPLOAD_FEE_LABEL} purpose={`Upload "${form.title}"`}
      onSuccess={txnId=>{
        onUpload({id:genId(),title:form.title,artist:currentUser.name,artistId:currentUser.id,
          genre:form.genre,plays:0,likes:0,albumName:form.albumName,isExplicit:form.isExplicit,
          cover:cover||`https://picsum.photos/seed/${genId()}/300/300`,
          audioUrl:audioUrl||null,duration:audioDuration?fmtTime(audioDuration):"—",
          durationSecs:audioDuration||0,trending:false,
...
// ─── ADMIN PANEL ──────────────────────────────────────────────────────────────
function AdminPanel({onClose,songs,setSongs,artists,setRegisteredUsers,toast}){
  ...
  const approve=id=>{setSongs(ss=>ss.map(s=>s.id===id?{...s,status:"Approved"}:s));toast("Approved ✅","success");};
  const reject=id=>{setSongs(ss=>ss.map(s=>s.id===id?{...s,status:"Rejected"}:s));toast("Rejected","info");};
  const remove=id=>{setSongs(ss=>ss.filter(s=>s.id!==id));toast("Removed","info");};
  const toggleTrend=id=>setSongs(ss=>ss.map(s=>s.id===id?{...s,trending:!s.trending}:s));
  const verifyArtist=id=>{setRegisteredUsers(uu=>uu.map(u=>u.id===id?{...u,verified:true,verificationStatus:"Approved"}:u));toast("Artist verified ✅","success");};
  ...
  if(!authed)return(
    <Modal onClose={onClose} maxWidth={320}>
      <div style={{textAlign:"center",padding:"8px 0"}}>
        ...
        <h2 style={{color:T.text,fontFamily:"'Syne',sans-serif",fontSize:17,marginBottom:5}}>Admin Access</h2>
        <p style={{color:T.sub,fontSize:12,marginBottom:20}}>Restricted to administrators</p>
        <input type="password" style={{...inp(),textAlign:"center",marginBottom:8,letterSpacing:4}} value={pw} onChange={e=>setPw(e.target.value)} onKeyDown={e=>e.key==="Enter"&&(pw==="admin123"?setAuthed(true):setErr("Wrong password"))} placeholder="••••••••" autoFocus/>
        {err&&<p style={{color:T.orange,fontSize:12,marginBottom:8}}>{err}</p>}
        <Btn full onClick={()=>pw==="admin123"?setAuthed(true):setErr("Wrong password")}>Enter</Btn>
        <p style={{color:T.mute,fontSize:10,marginTop:10}}>Demo: <strong style={{color:T.sub}}>admin123</strong></p>
      </div>
    </Modal>
  );
...
export default function MWPlay(){
  ...
  return(
    <div style={{fontFamily:"'DM Sans',sans-serif",background:T.bg,color:T.text,minHeight:"100vh",position:"relative",overflow:"hidden"}}>
            ...
            <section style={{padding:"24px 16px"}}>
              {currentUser?<>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  ...
                  <BtnGhost onClick={()=>setModal("admin")} style={{padding:10,fontSize:11,color:T.mute}}>⚙ Admin Panel</BtnGhost>
                  <button style={{background:"none",border:`1px solid ${T.border}`,borderRadius:8,padding:"10px 18px",fontSize:12,fontWeight:600,cursor:"pointer",color:T.mute,fontFamily:"'DM Sans',sans-serif"}} onClick={()=>{if(audioRef.current){audioRef.current.pause();audioRef.current.src="";}setCurrentUser(null);setPage("home");setCurrentSong(null);setIsPlaying(false);toast("Signed out");}}>Sign out</button>
                </div>
      ...
      {modal==="admin"&&<AdminPanel onClose={()=>setModal(null)} songs={songs} setSongs={setSongs} artists={registeredUsers} setRegisteredUsers={setRegisteredUsers} toast={toast}/>}
