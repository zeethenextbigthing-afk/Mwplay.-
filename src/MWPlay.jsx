...
import {
  authSignUp, authVerifyOtp, authSignIn, authSignOut, authResendOtp, authResetPassword,
  db, isSupabaseReady, supabase, getSession,
...
// RLS policy or table is blocking an operation.
function fullErrMsg(e){
  const parts=[e.message||"please try again"];
...
// Get the actual logged-in Supabase user id for this browser session.
// Throws a friendly error if the session is missing/expired so uploads
// don't silently fail an RLS check with a mismatched artist_id.
async function requireAuthUserId(currentUser){
  if(!isSupabaseReady) return currentUser.id;
  const session=await getSession();
  if(!session?.user) throw new Error("Your session has expired. Please log out and log back in, then try again.");
...
function useToast(){
  ...
  const show=useCallback((msg,type="info")=>{
    ...
    setTimeout(()=>setToasts(t=>t.filter(x=>x.id!==id)),type==="error"?10000:3400);
  },[]);
  return {toasts,show};
}
...
function useNotifications(currentUser) {
  const [notifs, setNotifs] = useState(() => {
    ...
    return LS.get("notifs_"+currentUser.id, []);
  });
  const unread = notifs.filter(n => !n.read).length;

  // Reload notifications when user changes (login/logout/switch account)
  useEffect(()=>{
    if(!currentUser?.id){setNotifs([]);return;}
    setNotifs(LS.get("notifs_"+currentUser.id,[]));
  },[currentUser?.id]);

  const addNotif = useCallback((type, msg, data={}) => {
    if(!currentUser?.id) return; // Don't add notifs when logged out
...
function usePlaylists(userId) {
  ...
  useEffect(()=>{
    ...
    let active=true;
    db.getPlaylists(userId).then(rows=>{
      if(active){setPlaylists(rows);LS.set(key,rows);}
    }).catch(()=>{});
    return()=>{active=false;};
  },[userId]);
  ...
  const createPlaylist = async (name, description="") => {
    ...
    if(isSupabaseReady&&userId){
      try{
        const saved=await db.createPlaylist(userId,name,description);
        // Replace temp id with real UUID from Supabase
        setPlaylists(ps=>ps.map(p=>p.id===pl.id?{...p,id:saved.id}:p));
        return saved;
      }catch(e){console.warn("Playlist create failed:",e);}
    }
    return pl;
  };
  ...
  const deletePlaylist = (id) => {
    ...
    if(isSupabaseReady)db.deletePlaylist(id).catch(()=>{});
  };

  const renamePlaylist = (id, name, description) => {
    save(playlists.map(p=>p.id===id?{...p,name,description}:p));
    if(isSupabaseReady)db.updatePlaylist(id,{name,description}).catch(()=>{});
  };

  const addSong = (playlistId, songId) => {
    setPlaylists(ps=>{
      const updated=ps.map(p=>p.id===playlistId?{...p,songs:p.songs.includes(songId)?p.songs:[...p.songs,songId]}:p);
      LS.set(key,updated);
      return updated;
    });
    if(isSupabaseReady){
      const pl=playlists.find(p=>p.id===playlistId);
      const pos=pl?pl.songs.length:0;
      db.addSongToPlaylist(playlistId,songId,pos).catch(()=>{});
    }
  };

  const removeSong = (playlistId, songId) => {
    setPlaylists(ps=>{
      const updated=ps.map(p=>p.id===playlistId?{...p,songs:p.songs.filter(s=>s!==songId)}:p);
      LS.set(key,updated);
      return updated;
    });
    if(isSupabaseReady)db.removeSongFromPlaylist(playlistId,songId).catch(()=>{});
  };
...
function AlbumUploadModal({ onClose, currentUser, onAlbumUpload, toast }) {
  ...
  const submit = async txnId => {
    const albumId = genId();
    toast(`Submitting "${albumInfo.title}"…`,"info");
    try{
      const authUserId=await requireAuthUserId(currentUser);
      const newSongs=[];
      const fileJobs=[]; // store file upload jobs for background processing
      for(let i=0;i<tracks.length;i++){
        const t=tracks[i];
        const id=genId();
        const song={
          id, albumId, albumName: albumInfo.title,
          title: t.title||`Track ${i+1}`, artist: currentUser.name, artistId: authUserId,
          genre: albumInfo.genre, cover:`https://picsum.photos/seed/${albumId}/300/300`,
          audioUrl: null,
      ...
      onClose();
      // Upload all audio + cover files in background
      if(isSupabaseReady){
...
function VerificationModal({ currentUser, onClose, onSubmit, toast }) {
  const [form, setForm] = useState({ fullName:"", phone:"", socialLink:"", reason:"", hasLabel:false, labelName:"" });
  const [step, setStep] = useState(1);
  const [submitted, setSubmitted] = useState(false);
  const up = (k,v) => setForm(f=>({...f,[k]:v}));

  const submit = () => {
    const request = { ...form, artistId: currentUser.id, status:"Pending", submittedAt: new Date().toISOString(), id: genId() };
    onSubmit(request);
    setSubmitted(true);
  ...
  return (
    <Modal onClose={onClose} maxWidth={460}>
      ...
      {step===1&&<>
        ...
        <Field label="Full name"><input style={inp()} value={form.fullName} onChange={e=>up("fullName",e.target.value)} placeholder="Your real name"/></Field>
        <Field label="Phone number" hint="We may WhatsApp you to confirm"><input style={inp()} value={form.phone} onChange={e=>up("phone",e.target.value)} placeholder="+265 8XX XXX XXX" type="tel"/></Field>
        <Field label="Facebook / Instagram / TikTok link" hint="Your official artist page"><input style={inp()} value={form.socialLink} onChange={e=>up("socialLink",e.target.value)} placeholder="https://facebook.com/yourname"/></Field>
        <Btn full onClick={()=>{
          if(!form.fullName||!form.phone){toast("Enter your name and phone number","error");return;}
          setStep(2);
        }}>Continue →</Btn>
      </>}

      {step===2&&<>
        <Field label="Tell us about yourself" hint="Your music career, releases, achievements...">
          <textarea style={{...inp(),height:100,resize:"none"}} value={form.reason} onChange={e=>up("reason",e.target.value)} placeholder="e.g. I'm a Malawian Afrobeat artist with 3 released singles on Audiomack and 10,000+ streams..."/>
        </Field>
    ...
    if(resetSent)return(
      <Modal onClose={onClose} maxWidth={380}>
        <div style={{textAlign:"center",padding:"16px 0"}}>
          ...
          <h2 style={{color:T.text,fontFamily:"'Syne',sans-serif",fontSize:18,marginBottom:8}}>Check your email</h2>
          <p style={{color:T.sub,fontSize:13,lineHeight:1.7,marginBottom:20}}>We sent a password reset link to <strong style={{color:T.blue}}>{form.email}</strong>. Tap the link in that email to set a new password.</p>
          <BtnGhost full onClick={()=>{setMode("login");setResetSent(false);}}>Back to Sign in</BtnGhost>
        </div>
      </Modal>
    );
    return(
      <Modal onClose={onClose} maxWidth={380}>
        <div style={{marginBottom:28,display:"flex",justifyContent:"center"}}><Logo/></div>
        <h2 style={{color:T.text,fontFamily:"'Syne',sans-serif",fontSize:18,marginBottom:6}}>Reset password</h2>
        <p style={{color:T.sub,fontSize:12,marginBottom:20}}>Enter your email and we'll send you a reset link.</p>
        ...
        <Field label="Email"><input style={inp()} type="email" value={form.email} onChange={e=>up("email",e.target.value)} placeholder="you@example.com" autoFocus/></Field>
        <Btn full loading={loading} onClick={async()=>{
          if(!form.email.includes("@"))return setError("Enter a valid email.");
          setLoading(true);setError("");
          try{
            await authResetPassword(form.email);
            setResetSent(true);
          }catch(e){setError(e.message||"Failed to send reset email.");}
          finally{setLoading(false);}
        }}>Send reset link</Btn>
        <p style={{textAlign:"center",marginTop:14}}>
          <button style={{background:"none",border:"none",color:T.blue,cursor:"pointer",fontSize:12}} onClick={()=>{setMode("login");setError("");}}>← Back to Sign in</button>
        </p>
      </Modal>
    );
  }
    ...
    if(mode==="signup"){
      ...
      try{
        if(isSupabaseReady){
          ...
          // Fetch full profile from Supabase (includes avatar, verified, bio etc.)
          let profile={
            id:user.id,
          ...
          try{
            ...
            if(data)profile={...profileFromDb(data),liked:[],following:[]};
          }catch(e){}
      ...
      <div style={{background:T.bg1,border:`1px solid ${T.border}`,borderRadius:20,padding:"32px 26px",width:"100%",maxWidth:400,maxHeight:"92vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
        ...
        {mode==="login"&&(
          <p style={{textAlign:"center",marginTop:10}}>
            <button style={{background:"none",border:"none",color:T.mute,cursor:"pointer",fontSize:11}} onClick={()=>setMode("forgot")}>
              Forgot password?
            </button>
          </p>
        )}
        <p style={{textAlign:"center",color:T.sub,fontSize:12,marginTop:8}}>
...
function UploadModal({onClose,currentUser,onUpload,toast,onUpdateSong}){
  const [step,setStep]=useState(1);
...
function Player({song,isPlaying,onPlayPause,onNext,onPrev,progress,duration,onSeek,volume,onVolume,onOpen,shuffle,onShuffle,repeat,onRepeat}){
  ...
  if(!song)return null;
  const secs=progress*(duration||0);
  const pct=`${progress*100}%`;

  // ── Full-screen player ────────────────────────────────────────────────────
  if(expanded)return(
    <div style={{position:"fixed",inset:0,zIndex:500,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      {/* Blurred artwork background */}
      <div style={{position:"absolute",inset:0,backgroundImage:`url(${song.cover})`,backgroundSize:"cover",backgroundPosition:"center",filter:"blur(40px) brightness(0.3)",transform:"scale(1.15)"}}/>
      <div style={{position:"absolute",inset:0,background:"linear-gradient(to bottom, rgba(0,0,0,.4) 0%, rgba(7,10,16,.85) 60%, rgba(7,10,16,1) 100%)"}}/>

      {/* Content */}
      <div style={{position:"relative",flex:1,display:"flex",flexDirection:"column",padding:"0 24px",paddingTop:"env(safe-area-inset-top,0px)"}}>

        {/* Header */}
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",paddingTop:16,marginBottom:32}}>
          <button onClick={()=>setExpanded(false)} style={{background:"none",border:"none",cursor:"pointer",color:T.sub,padding:8,display:"flex"}}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="6 9 12 15 18 9"/></svg>
          </button>
          <div style={{textAlign:"center"}}>
            <div style={{color:T.mute,fontSize:10,letterSpacing:2,fontWeight:600,textTransform:"uppercase"}}>Now Playing</div>
          </div>
          <button onClick={()=>onOpen&&onOpen(song)} style={{background:"none",border:"none",cursor:"pointer",color:T.sub,padding:8,display:"flex"}}>
        ...
        <div style={{display:"flex",justifyContent:"center",marginBottom:36}}>
          <div style={{position:"relative",width:"min(72vw, 300px)",aspectRatio:"1",borderRadius:16,overflow:"hidden",boxShadow:`0 24px 64px rgba(0,0,0,.6), 0 0 0 1px rgba(255,255,255,.06)`}}>
            <img src={song.cover} alt={song.title} style={{width:"100%",height:"100%",objectFit:"cover"}}/>
            {isPlaying&&<div style={{position:"absolute",bottom:10,right:10}}><Eq active={isPlaying} size={16} color="#fff"/></div>}
          </div>
        </div>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24}}>
          <div style={{flex:1,minWidth:0}}>
            <div style={{color:T.text,fontSize:20,fontWeight:700,fontFamily:"'Syne',sans-serif",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",marginBottom:4}}>{song.title}</div>
            <div style={{color:T.sub,fontSize:14}}>{song.artist}{song.albumName&&<span style={{color:T.mute}}> · {song.albumName}</span>}</div>
          </div>
          <div style={{display:"flex",gap:8,marginLeft:12,flexShrink:0}}>
            {song.isExplicit&&<span style={{background:T.mute,color:T.bg,fontSize:9,fontWeight:700,padding:"2px 5px",borderRadius:3}}>E</span>}
          </div>
        </div>

        {/* Progress bar */}
        <div style={{marginBottom:8}}>
          <div style={{height:4,background:"rgba(255,255,255,.15)",borderRadius:2,cursor:"pointer",position:"relative"}}
            onClick={e=>{const r=e.currentTarget.getBoundingClientRect();onSeek((e.clientX-r.left)/r.width);}}>
            <div style={{position:"absolute",top:0,left:0,height:"100%",borderRadius:2,background:`linear-gradient(90deg,${T.blue},${T.green})`,width:pct,transition:"width .5s linear"}}/>
            <div style={{position:"absolute",top:"50%",transform:"translateY(-50%)",left:pct,marginLeft:-6,width:12,height:12,borderRadius:"50%",background:"#fff",boxShadow:"0 2px 8px rgba(0,0,0,.4)",transition:"left .5s linear"}}/>
          </div>
          <div style={{display:"flex",justifyContent:"space-between",marginTop:8}}>
            <span style={{color:T.mute,fontSize:11,fontFamily:"'DM Mono',monospace"}}>{fmtTime(secs)}</span>
            <span style={{color:T.mute,fontSize:11,fontFamily:"'DM Mono',monospace"}}>{song.duration||"—"}</span>
          </div>
        </div>

        {/* Controls */}
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:28}}>
          <button onClick={onShuffle} style={{background:"none",border:"none",cursor:"pointer",padding:8,color:shuffle?T.blue:T.sub,display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
            ...
            {shuffle&&<div style={{width:4,height:4,borderRadius:"50%",background:T.blue}}/>}
          </button>
          <button onClick={onPrev} style={{background:"none",border:"none",cursor:"pointer",padding:8,color:T.text,display:"flex"}}>
            <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor"><polygon points="19 20 9 12 19 4 19 20"/><line x1="5" y1="19" x2="5" y2="5" stroke="currentColor" strokeWidth="2" fill="none"/></svg>
          </button>
          <button onClick={onPlayPause} style={{width:64,height:64,borderRadius:"50%",background:"#fff",border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:`0 8px 32px rgba(0,0,0,.4)`,flexShrink:0}}>
            {isPlaying
              ?<svg width="22" height="22" viewBox="0 0 24 24" fill={T.bg}><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg>
              :<svg width="22" height="22" viewBox="0 0 24 24" fill={T.bg} style={{marginLeft:3}}><polygon points="5 3 19 12 5 21 5 3"/></svg>}
          </button>
          <button onClick={onNext} style={{background:"none",border:"none",cursor:"pointer",padding:8,color:T.text,display:"flex"}}>
            <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor"><polygon points="5 4 15 12 5 20 5 4"/><line x1="19" y1="5" x2="19" y2="19" stroke="currentColor" strokeWidth="2" fill="none"/></svg>
          </button>
          <button onClick={onRepeat} style={{background:"none",border:"none",cursor:"pointer",padding:8,color:repeat?T.blue:T.sub,display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
            ...
            {repeat&&<div style={{width:4,height:4,borderRadius:"50%",background:T.blue}}/>}
          </button>
        </div>

        {/* Volume */}
        <div style={{display:"flex",alignItems:"center",gap:10,paddingBottom:24}}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={T.mute} strokeWidth="2"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/></svg>
          <input type="range" min={0} max={1} step={0.01} value={volume} onChange={e=>onVolume(parseFloat(e.target.value))}
            style={{flex:1,accentColor:T.blue,cursor:"pointer",height:4}}/>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={T.mute} strokeWidth="2"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M19.07 4.93a10 10 0 010 14.14M15.54 8.46a5 5 0 010 7.07"/></svg>
        </div>
      </div>
    </div>
  );
  ...
  return(
    <div style={{position:"fixed",bottom:60,left:0,right:0,zIndex:200}}>
      {/* Progress bar at top of mini player */}
      <div style={{height:2,background:T.mute}}>
        <div style={{height:"100%",background:`linear-gradient(90deg,${T.blue},${T.green})`,width:pct,transition:"width .5s linear"}}/>
      </div>
      <div style={{background:`rgba(11,15,26,.97)`,backdropFilter:"blur(24px)",borderTop:`1px solid ${T.border}`,display:"flex",alignItems:"center",padding:"8px 12px",gap:10}}
        onClick={()=>setExpanded(true)}>
        {/* Cover */}
        <img src={song.cover} alt="" style={{width:42,height:42,borderRadius:8,objectFit:"cover",flexShrink:0,boxShadow:`0 4px 16px rgba(0,0,0,.4)`}}/>
        {/* Info */}
        <div style={{flex:1,minWidth:0}}>
          <div style={{color:T.text,fontSize:13,fontWeight:600,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{song.title}</div>
          <div style={{color:T.sub,fontSize:11,display:"flex",alignItems:"center",gap:5}}>
            {song.artist}
            {isPlaying&&<Eq active size={9} color={T.blue}/>}
          </div>
        </div>
        {/* Controls — stop propagation so tapping prev/play/next doesn't expand */}
        <div style={{display:"flex",alignItems:"center",gap:4}} onClick={e=>e.stopPropagation()}>
          <button onClick={onPrev} style={{background:"none",border:"none",cursor:"pointer",padding:6,color:T.mute,display:"flex"}}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><polygon points="19 20 9 12 19 4 19 20"/><line x1="5" y1="19" x2="5" y2="5" stroke="currentColor" strokeWidth="2" fill="none"/></svg>
          </button>
          <button onClick={onPlayPause} style={{width:36,height:36,borderRadius:"50%",background:T.blue,border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:`0 4px 12px ${T.blue}66`}}>
            {isPlaying
              ?<svg width="12" height="12" viewBox="0 0 24 24" fill="#fff"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg>
              :<svg width="12" height="12" viewBox="0 0 24 24" fill="#fff" style={{marginLeft:1}}><polygon points="5 3 19 12 5 21 5 3"/></svg>}
          </button>
          <button onClick={onNext} style={{background:"none",border:"none",cursor:"pointer",padding:6,color:T.mute,display:"flex"}}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><polygon points="5 4 15 12 5 20 5 4"/><line x1="19" y1="5" x2="19" y2="19" stroke="currentColor" strokeWidth="2" fill="none"/></svg>
          </button>
        </div>
      </div>
    </div>
...
// ─── SONG PAGE ────────────────────────────────────────────────────────────────
function SongPage({song,onPlay,isPlaying,isActive,currentUser,onLike,allSongs,toast,onShare,onDownload,onAddToPlaylist,addNotif,onFollow,artists,onAddToQueue}){
  const [showLyrics,setShowLyrics]=useState(false);
  const [comment,setComment]=useState("");
  const [localComments,setLocalComments]=useState(()=>LS.get("comments_"+song.id,[]));
  const [loadingComments,setLoadingComments]=useState(false);
  const isLiked=currentUser?.liked?.includes(song.id);
  const isFollowing=currentUser?.following?.includes(song.artistId);
  ...
  useEffect(()=>{
    ...
    db.getComments(song.id).then(rows=>{
      if(active){setLocalComments(rows);LS.set("comments_"+song.id,rows);}
    }).catch(()=>{}).finally(()=>{if(active)setLoadingComments(false);});
    return()=>{active=false;};
  },[song.id]);
  ...
  const postComment=async()=>{
    ...
    setComment("");
    // Optimistic update
    const temp={id:genId(),user:currentUser.name,text,time:new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),createdAt:new Date().toISOString(),userId:currentUser.id};
    setLocalComments(prev=>[temp,...prev]);
    addNotif&&addNotif("comment",`You commented on "${song.title}"`);
    if(isSupabaseReady){
      try{
        const saved=await db.postComment(currentUser.id,song.id,text);
        // Replace temp with real saved comment
        setLocalComments(prev=>prev.map(c=>c.id===temp.id?saved:c));
      }catch(e){
        // Revert on failure
        setLocalComments(prev=>prev.filter(c=>c.id!==temp.id));
        toast&&toast("Failed to post comment","error");
      }
    }
  };
  ...
  const deleteComment=async(id)=>{
    ...
    if(isSupabaseReady)db.deleteComment(id).catch(()=>{});
  };
    ...
    <div style={{padding:"20px 16px",maxWidth:680,margin:"0 auto"}}>
      <div style={{display:"flex",gap:20,flexWrap:"wrap",marginBottom:24}}>
        ...
        <div style={{flex:1,minWidth:180}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:song.albumName?4:12,flexWrap:"wrap"}}>
            <div style={{color:T.blue,fontSize:13,fontWeight:600}}>{song.artist}</div>
            {currentUser&&song.artistId!==currentUser.id&&(
              <button onClick={()=>{const a=artists?.find(x=>x.id===song.artistId)||{id:song.artistId,name:song.artist};onFollow&&onFollow(a);}}
                style={{padding:"3px 12px",borderRadius:20,border:`1px solid ${isFollowing?T.blue:T.border2}`,background:isFollowing?`${T.blue}18`:"transparent",color:isFollowing?T.blue:T.sub,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",transition:"all .15s"}}>
          ...
          <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
            ...
            <BtnGhost onClick={()=>onAddToQueue&&onAddToQueue(song)} style={{padding:"8px 12px"}} title="Add to queue">⏭</BtnGhost>
          </div>
        </div>
      </div>
      ...
      {currentUser
        ?<div style={{display:"flex",gap:8,marginBottom:14}}>
          <input style={{...inp(),flex:1}} value={comment} onChange={e=>setComment(e.target.value)} placeholder="Leave a comment..." onKeyDown={e=>e.key==="Enter"&&postComment()}/>
          <Btn onClick={postComment} disabled={!comment.trim()}>Post</Btn>
        </div>
        :<p style={{color:T.mute,fontSize:12,marginBottom:12}}>Sign in to comment.</p>}
      {loadingComments&&<p style={{color:T.mute,fontSize:12}}>Loading comments…</p>}
      {localComments.map(c=>(
        <div key={c.id} style={{display:"flex",gap:10,marginBottom:14,alignItems:"flex-start"}}>
          <div style={{width:32,height:32,borderRadius:"50%",background:`${T.blue}18`,border:`1px solid ${T.blueBorder}`,display:"flex",alignItems:"center",justifyContent:"center",color:T.blue,fontSize:12,fontWeight:700,flexShrink:0,overflow:"hidden"}}>
            {c.avatar?<img src={c.avatar} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:c.user[0]?.toUpperCase()}
          </div>
          <div style={{flex:1,minWidth:0}}>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2}}>
              <span style={{color:T.text,fontSize:12,fontWeight:600}}>{c.user}</span>
              <span style={{color:T.mute,fontSize:10}}>{c.time}</span>
              {currentUser?.id===c.userId&&<button onClick={()=>deleteComment(c.id)} style={{background:"none",border:"none",cursor:"pointer",color:T.mute,fontSize:10,padding:0,marginLeft:"auto"}}>✕</button>}
            </div>
            <div style={{color:T.sub,fontSize:13,lineHeight:1.5,wordBreak:"break-word"}}>{c.text}</div>
          </div>
        </div>
      ))}
      {localComments.length===0&&!loadingComments&&<p style={{color:T.mute,fontSize:13}}>Be the first to comment.</p>}
...
// ─── ARTIST DASHBOARD ─────────────────────────────────────────────────────────
function ArtistDashboard({onClose,currentUser,songs,onUpdateUser,toast,onVerify,verificationStatus,onRefresh}){
...
function AdminPanel({onClose,songs,setSongs,artists,setRegisteredUsers,toast,currentUser}){
  ...
  const verifyArtist=id=>{
    setRegisteredUsers(uu=>uu.map(u=>u.id===id?{...u,verified:true,verificationStatus:"Approved"}:u));
    toast("Artist verified ✅","success");
    if(isSupabaseReady)supabase.rpc('admin_verify_artist',{p_user_id:id}).then(()=>{
      // Refresh profiles so the badge shows immediately
      db.getProfiles().then(rows=>{
        setRegisteredUsers(rows.map(r=>{
          const existing=rows.find(p=>p.id===r.id);
          return profileFromDb(r,{liked:existing?.liked||[],following:existing?.following||[]});
        }));
      }).catch(()=>{});
    }).catch(()=>toast("Failed to sync verified status","error"));
  };
  ...
  useEffect(()=>{
    if(!isSupabaseReady||tab!=="verify")return;
    supabase.from('verif_queue').select('*').order('created_at',{ascending:false}).then(({data})=>{
      if(data)setVerifQueue(data);
    }).catch(()=>{});
  },[tab]);
  ...
  return(
    <Modal onClose={onClose} maxWidth={720} padding="0">
      ...
      <div style={{display:"flex",borderBottom:`1px solid ${T.border}`,padding:"0 22px",overflowX:"auto"}}>
        {[["dashboard","Dashboard"],["content","Tracks"],["pending",`Pending (${pending.length})`],["artists","Artists"],["verify",`Verify (${verifQueue.length})`]].map(([k,l])=>(
      ...
      <div style={{padding:"20px 22px",maxHeight:"72vh",overflowY:"auto"}}>
        ...
        {tab==="verify"&&(verifQueue.length===0
          ?<EmptyState title="No pending verifications" sub="No artist verification requests yet."/>
          :verifQueue.map(r=>(
            <div key={r.id} style={{background:T.bg2,border:`1px solid ${T.border}`,borderRadius:10,padding:"14px 16px",marginBottom:10}}>
              <div style={{marginBottom:10}}>
                <div style={{color:T.text,fontSize:13,fontWeight:600,marginBottom:2}}>{r.artist_name}</div>
                <div style={{color:T.sub,fontSize:11,marginBottom:2}}>📱 {r.phone}</div>
                {r.social&&<div style={{color:T.blue,fontSize:11,marginBottom:2}}>🔗 {r.social}</div>}
                {r.reason&&<div style={{color:T.sub,fontSize:11,lineHeight:1.5,marginBottom:6}}>"{r.reason}"</div>}
                <div style={{color:T.mute,fontSize:10}}>{new Date(r.created_at).toLocaleDateString()}</div>
              </div>
              <div style={{display:"flex",gap:8}}>
                <Btn flex={1} color={T.green} onClick={()=>{
                  verifyArtist(r.artist_id);
                  setVerifQueue(q=>q.filter(x=>x.id!==r.id));
                  supabase.from('verif_queue').delete().eq('id',r.id).catch(()=>{});
                  toast("Artist verified ✅","success");
                }}>✓ Verify</Btn>
                <BtnGhost flex={1} onClick={()=>{
                  setVerifQueue(q=>q.filter(x=>x.id!==r.id));
                  supabase.from('verif_queue').delete().eq('id',r.id).catch(()=>{});
...
// ─── LIBRARY PAGE ─────────────────────────────────────────────────────────────
function LibraryPage({songs,currentUser,playlists,onPlay,isPlaying,currentSong,onOpen,onCreatePlaylist,onDeletePlaylist,onRemoveSong,toast,setPage,recentlyPlayed,queue,onRemoveFromQueue,onClearQueue,topPlayed}){
  ...
  // Charts: top 10 by genre
  const genres=[...new Set(approvedSongs.map(s=>s.genre).filter(Boolean))];

  return(
    <section style={{padding:"0 0 20px"}}>
      <div style={{padding:"20px 16px 0"}}>
        <h2 style={{color:T.text,fontFamily:"'Syne',sans-serif",fontSize:17,marginBottom:16}}>Your Library</h2>
        <div style={{display:"flex",gap:0,background:T.bg2,borderRadius:10,border:`1px solid ${T.border}`,padding:3,marginBottom:4,overflowX:"auto"}}>
          {[["liked","♥ Liked"],["playlists","🎶 Playlists"],["following","👤 Following"],["recent","🕐 Recent"],["queue",`⏭ Queue${queue.length>0?` (${queue.length})`:""}`],["charts","📊 Charts"]].map(([k,l])=>(
            <button key={k} style={{flex:"0 0 auto",padding:"8px 10px",borderRadius:7,border:"none",cursor:"pointer",fontSize:11,fontWeight:700,background:tab===k?T.blue:"transparent",color:tab===k?"#fff":T.sub,transition:"all .18s",fontFamily:"'DM Sans',sans-serif",whiteSpace:"nowrap"}} onClick={()=>setTab(k)}>{l}</button>
          ))}
        </div>
      </div>
      ...
      {tab==="recent"&&(
        !recentlyPlayed||recentlyPlayed.length===0
          ?<EmptyState title="No recent plays" sub="Songs you play will appear here." icon={<span style={{fontSize:24}}>🕐</span>}/>
          :<div style={{padding:"0 16px",display:"flex",flexDirection:"column",gap:6}}>
            <p style={{color:T.mute,fontSize:11,marginBottom:4}}>{recentlyPlayed.length} recently played</p>
            {recentlyPlayed.map((s,i)=>(
              <div key={s.id+i} style={{display:"flex",gap:12,alignItems:"center",background:T.card,border:`1px solid ${currentSong?.id===s.id?T.blueBorder:T.border}`,borderRadius:8,padding:"10px 12px",cursor:"pointer"}} onClick={()=>onPlay(s)}>
                <img src={s.cover} alt="" style={{width:42,height:42,borderRadius:6,objectFit:"cover",flexShrink:0}}/>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{color:T.text,fontSize:13,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.title}</div>
                  <div style={{color:T.sub,fontSize:11}}>{s.artist}</div>
                </div>
                {currentSong?.id===s.id&&isPlaying&&<Eq active size={12} color={T.blue}/>}
              </div>
            ))}
          </div>
      )}
      ...
      {tab==="queue"&&(
        queue.length===0
          ?<EmptyState title="Queue is empty" sub="Add songs to your queue to play them next." icon={<span style={{fontSize:24}}>⏭</span>}/>
          :<div style={{padding:"0 16px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
              <p style={{color:T.mute,fontSize:11}}>{queue.length} song{queue.length!==1?"s":""} up next</p>
              <button onClick={onClearQueue} style={{background:"none",border:`1px solid ${T.border}`,borderRadius:6,color:T.mute,fontSize:11,padding:"4px 10px",cursor:"pointer"}}>Clear all</button>
            </div>
            {queue.map((s,i)=>(
              <div key={s.id} style={{display:"flex",gap:12,alignItems:"center",background:T.card,border:`1px solid ${T.border}`,borderRadius:8,padding:"10px 12px",cursor:"pointer",marginBottom:6}}>
                <span style={{color:T.mute,fontSize:11,width:16,flexShrink:0}}>{i+1}</span>
                <img src={s.cover} alt="" style={{width:40,height:40,borderRadius:6,objectFit:"cover",flexShrink:0}} onClick={()=>onPlay(s)}/>
                <div style={{flex:1,minWidth:0}} onClick={()=>onPlay(s)}>
                  <div style={{color:T.text,fontSize:13,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.title}</div>
                  <div style={{color:T.sub,fontSize:11}}>{s.artist}</div>
                </div>
                <button onClick={()=>onRemoveFromQueue(s.id)} style={{background:"none",border:"none",cursor:"pointer",color:T.mute,fontSize:16,padding:4}}>✕</button>
              </div>
            ))}
          </div>
      )}
      ...
      {tab==="charts"&&(
        <div style={{padding:"0 16px"}}>
          <div style={{marginBottom:24}}>
            <h3 style={{color:T.text,fontFamily:"'Syne',sans-serif",fontSize:14,fontWeight:700,marginBottom:12}}>🏆 Top 10 Most Played</h3>
            {topPlayed.length===0
              ?<EmptyState title="No plays yet" sub="Songs with the most plays will appear here."/>
              :topPlayed.map((s,i)=>(
                <div key={s.id} style={{display:"flex",gap:12,alignItems:"center",background:T.card,border:`1px solid ${i<3?T.orangeBorder:T.border}`,borderRadius:8,padding:"10px 12px",cursor:"pointer",marginBottom:6}} onClick={()=>onPlay(s)}>
                  <div style={{width:24,textAlign:"center",flexShrink:0}}>
                    {i===0?<span style={{fontSize:16}}>🥇</span>:i===1?<span style={{fontSize:16}}>🥈</span>:i===2?<span style={{fontSize:16}}>🥉</span>:<span style={{color:T.mute,fontSize:12,fontWeight:700}}>#{i+1}</span>}
                  </div>
                  <img src={s.cover} alt="" style={{width:40,height:40,borderRadius:6,objectFit:"cover",flexShrink:0}}/>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{color:T.text,fontSize:13,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.title}</div>
                  ...
                  <div style={{color:T.mute,fontSize:11,flexShrink:0}}>{s.plays||0} plays</div>
                </div>
              ))
            }
          </div>
          {genres.length>0&&<>
            <h3 style={{color:T.text,fontFamily:"'Syne',sans-serif",fontSize:14,fontWeight:700,marginBottom:12}}>🎵 Top by Genre</h3>
            {genres.map(g=>{
              const top=approvedSongs.filter(s=>s.genre===g).sort((a,b)=>b.plays-a.plays).slice(0,3);
              if(top.length===0)return null;
              return(
                <div key={g} style={{marginBottom:20}}>
                  <div style={{color:T.blue,fontSize:12,fontWeight:700,letterSpacing:.5,marginBottom:8}}>{g.toUpperCase()}</div>
                  {top.map((s,i)=>(
                    <div key={s.id} style={{display:"flex",gap:10,alignItems:"center",padding:"7px 0",borderBottom:`1px solid ${T.border}`,cursor:"pointer"}} onClick={()=>onPlay(s)}>
                      <span style={{color:T.mute,fontSize:11,width:16}}>#{i+1}</span>
                      <img src={s.cover} alt="" style={{width:34,height:34,borderRadius:4,objectFit:"cover"}}/>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{color:T.text,fontSize:12,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.title}</div>
                        <div style={{color:T.sub,fontSize:10}}>{s.artist}</div>
                      </div>
                      <span style={{color:T.mute,fontSize:10}}>{s.plays||0}</span>
                    </div>
                  ))}
                </div>
              );
...
export default function MWPlay(){
  ...
  const [recentlyPlayed,setRecentlyPlayed]=useState(()=>LS.get("recent_"+(LS.get("currentUser",null)?.id||""),[]));
  ...
  // Recently played — reload when user changes
  useEffect(()=>{
    if(currentUser?.id)setRecentlyPlayed(LS.get("recent_"+currentUser.id,[]));
    else setRecentlyPlayed([]);
  },[currentUser?.id]);

  const addToQueue=(song)=>{setQueue(q=>[...q.filter(s=>s.id!==song.id),song]);toast(`Added "${song.title}" to queue`,"info");};
  ...
  const playSong=useCallback(song=>{
    if(currentSong?.id===song.id){setIsPlaying(p=>!p);}
      ...
      // Track recently played (per user, max 20)
      if(currentUser?.id){
        setRecentlyPlayed(prev=>{
          const updated=[song,...prev.filter(s=>s.id!==song.id)].slice(0,20);
          LS.set("recent_"+currentUser.id,updated);
          return updated;
        });
      }
      // Remove from queue if playing from queue
  ...
  const nextSong=useCallback(()=>{
    ...
    if(queue.length>0){const next=queue[0];playSong(next);return;}
    if(shuffle){const pool=approvedSongs.filter(s=>s.id!==currentSong?.id);if(pool.length>0){const s=pool[Math.floor(Math.random()*pool.length)];playSong(s);return;}}
    const i=approvedSongs.findIndex(s=>s.id===currentSong?.id);
    const n=approvedSongs[(i+1)%approvedSongs.length];
    if(n)playSong(n);
  },[approvedSongs,currentSong,shuffle,queue,playSong]);
  ...
  // Refresh current user's profile from Supabase (call after verification/profile updates)
  const refreshCurrentUser = async () => {
    if(!isSupabaseReady||!currentUser?.id)return;
    try{
      const {data}=await supabase.from('profiles').select('*').eq('id',currentUser.id).single();
      if(data){
        const updated=profileFromDb(data,{liked:currentUser.liked,following:currentUser.following});
        setCurrentUser(updated);
        setRegisteredUsers(uu=>uu.map(u=>u.id===updated.id?{...u,...updated}:u));
      }
    }catch(e){}
  };
  ...
  // Expose song mutators so ArtistDashboard (a modal) can call them
  useEffect(()=>{
    window.__mwDeleteSong=(id)=>{
      setSongs(ss=>ss.filter(s=>s.id!==id));
    };
    window.__mwUpdateSong=(id,patch)=>{
      setSongs(ss=>ss.map(s=>s.id===id?{...s,...patch}:s));
    };
    return()=>{delete window.__mwDeleteSong;delete window.__mwUpdateSong;};
  },[]);

  const handleVerifySubmit=async request=>{
    ...
    if(isSupabaseReady){
      try{
        const {error}=await supabase.from('verif_queue').insert([{
          artist_id:currentUser.id,
          artist_name:request.fullName||currentUser.name,
          phone:request.phone||"",
          social:request.socialLink||"",
          reason:request.reason||"",
        }]);
        if(error)throw error;
        setTimeout(refreshCurrentUser,1000);
      }catch(e){
        console.error("Verify submit error:",e);
        toast("Saved locally but failed to sync: "+fullErrMsg(e),"error");
      }
    }
  };
  ...
  const topPlayed=[...approvedSongs].sort((a,b)=>b.plays-a.plays).slice(0,10);
    ...
    <div style={{fontFamily:"'DM Sans',sans-serif",background:T.bg,color:T.text,minHeight:"100vh",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{position:"relative",height:"100vh",overflow:"hidden"}}>
          ...
          {search?(
            <section style={{padding:"20px 16px"}}>
              {(()=>{
                ...
                const tracks=approvedSongs.filter(s=>
                  s.title?.toLowerCase().includes(q)||
                  s.artist?.toLowerCase().includes(q)||
                  s.genre?.toLowerCase().includes(q)||
                  s.albumName?.toLowerCase().includes(q)
                );
                const arts=artistUsers.filter(a=>
            ...
            <>
              ...
              <footer style={{background:T.bg,borderTop:`1px solid ${T.border}`,padding:"28px 16px",textAlign:"center"}}>
                ...
                <div style={{display:"flex",justifyContent:"center",gap:16,marginBottom:10}}>
                  <button onClick={()=>setModal("terms")} style={{background:"none",border:"none",color:T.mute,fontSize:11,cursor:"pointer",textDecoration:"underline"}}>Terms of Service</button>
                  <button onClick={()=>setModal("privacy")} style={{background:"none",border:"none",color:T.mute,fontSize:11,cursor:"pointer",textDecoration:"underline"}}>Privacy Policy</button>
                  <button onClick={()=>setModal("contact")} style={{background:"none",border:"none",color:T.mute,fontSize:11,cursor:"pointer",textDecoration:"underline"}}>Contact</button>
                </div>
            ...
            <section style={{padding:"24px 16px"}}>
              ...
              {artistUsers.length===0
                ...
                :<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:12}}>
                  {artistUsers.map(a=>{
                    const isFollowingA=currentUser?.following?.includes(a.id);
                    return(
                    <div key={a.id} className="mwcard" style={{background:T.card,borderRadius:10,padding:"16px 12px",textAlign:"center",border:`1px solid ${T.border}`,cursor:"pointer"}} onClick={()=>setOpenArtist(a)}>
                      ...
                      <div style={{color:T.mute,fontSize:10,marginBottom:8}}>{songs.filter(s=>s.artistId===a.id&&s.status==="Approved").length} tracks · {fmtNum(a.followers||0)} followers</div>
                      {currentUser&&a.id!==currentUser.id&&(
                        <button onClick={e=>{e.stopPropagation();handleFollow(a);}}
                          style={{width:"100%",padding:"5px 0",borderRadius:6,border:`1px solid ${isFollowingA?T.blue:T.border2}`,background:isFollowingA?`${T.blue}18`:"transparent",color:isFollowingA?T.blue:T.sub,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",transition:"all .15s"}}>
                          {isFollowingA?"Following ✓":"+ Follow"}
                        </button>
                      )}
                    </div>
                    );
            ...
            <LibraryPage songs={songs} currentUser={currentUser} playlists={playlists} onPlay={playSong} isPlaying={isPlaying} currentSong={currentSong} onOpen={setOpenSong} onCreatePlaylist={createPlaylist} onDeletePlaylist={deletePlaylist} onRemoveSong={removeFromPlaylist} toast={toast} setPage={setPage} recentlyPlayed={recentlyPlayed} queue={queue} onRemoveFromQueue={removeFromQueue} onClearQueue={clearQueue} topPlayed={topPlayed}/>
          ):page==="profile"?(
            <section style={{padding:"24px 16px"}}>
              {currentUser?<>
        ...
        <PageSlide show={showSongPage}>
          ...
          {openSong&&<SongPage song={openSong} onPlay={playSong} isPlaying={isPlaying} isActive={currentSong?.id===openSong.id} currentUser={currentUser} onLike={handleLike} allSongs={approvedSongs} toast={toast} onShare={setShareTarget} onDownload={handleDownload} onAddToPlaylist={setAddToPlaylistTarget} addNotif={addNotif} onFollow={handleFollow} artists={registeredUsers} onAddToQueue={addToQueue}/>}
        </PageSlide>

        {/* ARTIST PAGE */}
        <PageSlide show={showArtistPage}>
          <BackNav label="ARTIST" onBack={()=>setOpenArtist(null)}/>
          {openArtist&&<ArtistProfilePage artist={registeredUsers.find(u=>u.id===openArtist.id)||openArtist} onPlay={playSong} songs={songs} isPlaying={isPlaying} currentSong={currentSong} currentUser={currentUser} onFollowToggle={handleFollow} onShare={setShareTarget} onDownload={handleDownload} toast={toast}/>}
        </PageSlide>
      </div>
      ...
      {modal==="upload"&&currentUser?.role==="artist"&&<UploadModal onClose={()=>setModal(null)} currentUser={currentUser} onUpload={s=>{setSongs(ss=>[...ss,s]);addNotif("upload",`"${s.title}" submitted for review`);}} toast={toast} onUpdateSong={updated=>setSongs(ss=>ss.map(s=>s.id===updated.id?{...s,...updated}:s))}/>}
      ...
      {modal==="dashboard"&&currentUser?.role==="artist"&&<ArtistDashboard onClose={()=>setModal(null)} currentUser={currentUser} songs={songs} onUpdateUser={handleUpdateUser} toast={toast} onVerify={()=>{setModal(null);setShowVerify(true);}} verificationStatus={currentUser.verificationStatus} onRefresh={refreshCurrentUser}/>}
      ...
      {modal==="terms"&&<LegalModal title="Terms of Service" onClose={()=>setModal(null)} content={TERMS_CONTENT}/>}
      {modal==="privacy"&&<LegalModal title="Privacy Policy" onClose={()=>setModal(null)} content={PRIVACY_CONTENT}/>}
      {modal==="contact"&&<LegalModal title="Contact Us" onClose={()=>setModal(null)} content={CONTACT_CONTENT}/>}
    </div>
  );
}
...
// ─── LEGAL CONTENT ────────────────────────────────────────────────────────────
const TERMS_CONTENT = `
**MW Play Terms of Service**
Last updated: June 2026

**1. Acceptance**
By using MW Play, you agree to these terms. If you do not agree, please do not use the platform.

**2. Artist Uploads**
Artists pay a one-time upload fee of MWK 1,000 per track. Songs are reviewed and approved by the MW Play team before going live. We reserve the right to reject content that violates our guidelines.

**3. Downloads**
Listeners pay MWK 500 per track download. Downloads are for personal use only and may not be redistributed or sold.

**4. Prohibited Content**
You may not upload: content you don't own, explicit content without marking it as such, hate speech, or content that violates Malawian law.

**5. Intellectual Property**
Artists retain all rights to their music. By uploading to MW Play, you grant us a non-exclusive license to stream and distribute your music on the platform.

**6. Payments**
All payments are made via TNM Mpamba or Airtel Money to 0884907615. Payments are non-refundable once a song is approved or a download is delivered.

**7. Termination**
We reserve the right to remove content or suspend accounts that violate these terms.

**8. Contact**
For questions, WhatsApp us at 0884907615 or email zeethenextbigthing@gmail.com.
`;

const PRIVACY_CONTENT = `
**MW Play Privacy Policy**
Last updated: June 2026

**1. Information We Collect**
We collect your name, email address, and payment reference numbers when you sign up or make payments.

**2. How We Use Your Information**
Your information is used to manage your account, process payments, and improve the platform. We do not sell your personal data.

**3. Data Storage**
Your data is stored securely using Supabase, hosted in the European Union. Audio files are stored in Supabase Storage.

**4. Cookies**
MW Play uses local storage to remember your preferences and session. No third-party tracking cookies are used.

**5. Your Rights**
You can request deletion of your account and data by contacting us. We will process requests within 30 days.

**6. Third Parties**
We use Supabase for database and storage. Their privacy policy applies to data processed through their services.

**7. Children**
MW Play is not intended for users under 13 years of age.

**8. Contact**
For privacy concerns, contact us at zeethenextbigthing@gmail.com.
`;
...
📱 **WhatsApp / Call**
0884907615 (Given — Upcoming Zee)

📧 **Email**
zeethenextbigthing@gmail.com

🌍 **Location**
Mulanje, Malawi

**For Artists:**
Questions about uploads, payments, or verification? WhatsApp us directly for the fastest response.

**For Listeners:**
Questions about downloads or your account? Send us an email and we'll get back to you within 24 hours.

**Payment Issues:**
If you sent payment but your song/download wasn't processed, send your M-Pamba/Airtel reference number via WhatsApp with the song title and we'll sort it out immediately.
`;
      ...
      <div style={{maxHeight:"70vh",overflowY:"auto",padding:"4px 0"}}>
        {content.trim().split("\n").map((line,i)=>{
          ...
          if(line.startsWith("**")&&line.endsWith("**"))
            return <p key={i} style={{color:T.text,fontWeight:700,fontSize:13,marginBottom:4,marginTop:i>0?12:0}}>{line.replace(/\*\*/g,"")}</p>;
          return <p key={i} style={{color:T.sub,fontSize:12,lineHeight:1.7}}>{line}</p>;
        })}
      </div>
    </Modal>
  );
