...
export const db = {
  ...
  async updateOwnProfile(id, updates) {
    if (!supabase) return null
    const { data, error } = await supabase.from('profiles').update(updates).eq('id', id).select().single()
    if (error) throw error
    return data
  ...
  async getPlaylists(userId) {
    ...
    return (data || []).map(p => ({
      id: p.id, name: p.name, description: p.description, cover: p.cover,
  ...
  async createPlaylist(userId, name, description = '') {
    ...
    const { data, error } = await supabase
      ...
      .select().single()
    if (error) throw error
    return { id: data.id, name: data.name, description: data.description, cover: data.cover, createdAt: data.created_at, songs: [] }
  },
  ...
  async postComment(userId, songId, text) {
    if (!supabase) return null
    const { data, error } = await supabase
      .from('comments')
      .insert([{ user_id: userId, song_id: songId, text }])
